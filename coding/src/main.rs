...
fn main() {
    ...
    // coded string "abc"
    /*
    let coded: [u32; 4] = [0x9fdd9158, 0x85715808, 0xac73323a, 0];
    ...
    */
    ...
    let mut decoded: [u8; 132] = [0; 132];
    decode(&coded, &mut decoded, &mut seed);
    println!("{}", from_utf8(&decoded[..]).unwrap());
    // test codgen output
    /*
    let mut s: u32 = 0x3e944b9f;
    assert_eq!(codgen(&mut s), 0x891432f9, "1111");
    assert_eq!(codgen(&mut s), 0x4aa1dccc, "2222");
    assert_eq!(codgen(&mut s), 0xc54270fa, "3333");
    assert_eq!(codgen(&mut s), 0x9885155f, "4444");
    assert_eq!(codgen(&mut s), 0xce83d1b8, "5555");
    */
}
...
fn decode(wordarr: &[u32], bytearr: &mut [u8], seed: &mut u32) -> u32 {
    ...
    if wordarr[0] == 0 {
        ...
        // two's complement
        let r: u32 = - (codgen(seed) as i32) as u32;
